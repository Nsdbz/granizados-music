...
function init() {
  ...
  setInterval(loadBlocked, 15000)  // se refresca sola cada 15s
}
...
// ─── VIDEOS NO DISPONIBLES EN PANTALLA ───────────────────────────────────────
...
async function loadBlocked() {
  try {
    ...
    el.innerHTML = list.map(v => `
      <div class="queue-row" style="border:1px solid rgba(255,45,155,0.2)">
        ${v.thumbnail
          ? `<img src="${v.thumbnail}" alt="" style="width:52px;height:38px;border-radius:7px;object-fit:cover;flex-shrink:0">`
          : '<div class="thumb-ph">🎵</div>'
        }
        <div class="q-info" style="flex:1;min-width:0">
          <div class="q-title" style="color:var(--pink)">${v.title}</div>
          <div style="font-size:.68rem;color:var(--muted);margin-top:2px">
            ID: ${v.videoId}
            ${v.errorCode != null ? ` · código ${v.errorCode}` : ''}
            · ${timeAgo(v.blockedAt)}
          </div>
        </div>
        <div style="display:flex;gap:5px;flex-shrink:0">
          <button class="btn-sm btn-ghost" title="Abrir en YouTube" onclick="window.open('https://www.youtube.com/watch?v=${v.videoId}','_blank')">▶</button>
